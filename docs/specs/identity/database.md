...
| テーブル | 説明 |
|---------|------|
| profiles | アカウント（Supabase Auth に紐づく） |
...
- 動作:
  1. profiles に INSERT（id = NEW.id, display_name は Auth メタデータから取得）
...
| account_id | uuid | NO | - | FK → profiles.id |
...
- PK: `id`
- FK: `account_id` → `profiles.id` (ON DELETE CASCADE)
